...
export class Finders {
  ...
  static reloadIngredientsFinder(NewTag) {
    ...
    for (let i = 0; i < FunctionalMasterList.masterList.length; i++) {
      ...
      for (
        let j = 0;
        j < FunctionalMasterList.masterList[i].ingredients.length;
        j++
      ) {
        let present = false;
        ...
        for (let k = 0; k < NewTag.ingredientsTaglist.length; k++) {
          if (
            NewTag.ingredientsTaglist[k] ==
            FunctionalMasterList.masterList[i].ingredients[
              j
  ...
  static reloadAppareilsFinder(NewTag) {
    ...
    for (let i = 0; i < FunctionalMasterList.masterList.length; i++) {
      ...
      for (let k = 0; k < NewTag.appareilsTaglist.length; k++) {
        if (
          NewTag.appareilsTaglist[k] ==
          FunctionalMasterList.masterList[i].appliance.toLowerCase()
        ) {
          present = true;
          break;
        }
    ...
    if (FilteredAppareilslist.length > 0) {
  ...
  static reloadUstensilesFinder(NewTag) {
    ...
    for (let i = 0; i < FunctionalMasterList.masterList.length; i++) {
      ...
      for (
        let j = 0;
        j < FunctionalMasterList.masterList[i].ustensiles.length;
        j++
      ) {
        let present = false;
        // comparer chaque ingredient a sa TagList si present =true
        for (let k = 0; k < NewTag.ustensilesTaglist.length; k++) {
          if (
            NewTag.ustensilesTaglist[k] ==
            FunctionalMasterList.masterList[i].ustensiles[j].toLowerCase()
          ) {
            present = true;
          }
        }
    ...
    if (FilteredUstensillist.length > 0) {
